...
'''
Script for getting the TV reconstruction metrics
'''
...
def run_experiment(
    ...
    method = 'tv'
    save_dir = f'{save_dir}{evaluation_set}/{task}/{n_views}_views_snr_{snr}_dB/{method}/weight_{weight}/delta_{delta}/'
    ...
    with torch.no_grad():
        for batch_idx in tqdm(range(num_batches), desc="Evaluating Batches"):
            ...
            tv_recon = ct_op.tv_recon(y_meas, weight=weight, delta=delta, num_subsets=num_subsets, num_tv=num_tv).unsqueeze(0)

            end_time = time.perf_counter()
            runtimes.append(end_time - start_time)
            
            reconstructions.append(tv_recon)
