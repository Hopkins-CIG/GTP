...
def run_experiment(
        ...
        lamda: float = 1e-1,
        num_samples: int = 1,
        step_size_scaling: str = 'improved', # Options are 'improved' or 'original' for the step size scaling in the data consistency step
                                       # 'original' follows the original DPS implementation, whereas 'improved' is our design choice
        # Device
        device: str = 'cuda:0',
        data_root: str = None,
    ...
    save_dir = f'{save_dir}{evaluation_set}/{task}/{n_views}_views_snr_{snr}_dB/dps/lamda_{lamda}/step_size_scaling_{step_size_scaling}/'
    ...
    def data_consistency(x, y_meas, diffusion, denoiser, y, sigma, sigma_next, x_T, lamda, dt, sig_t, sigma_meas, 
                         step_size_scaling, **kwargs):
        ...
        with torch.set_grad_enabled(True):
            ...
            if step_size_scaling == 'original':
                L = torch.linalg.norm(diff)
                L_grad = torch.autograd.grad(outputs=L, inputs=x)[0]
                step_size = lamda
                x = x - step_size * L_grad

            if step_size_scaling == 'improved':
                L = 1/(2*sigma_meas**2) * torch.linalg.norm(diff)**2
                L_grad = torch.autograd.grad(outputs=L, inputs=x)[0]
                step_size = lamda / torch.linalg.norm(L_grad)
                x = x - step_size * L_grad
    ...
    with torch.no_grad():
        for batch_idx in tqdm(range(len(test_ct_slices)), desc="Evaluating Batches"):
            ...
            init_cond = torch.randn((num_samples, 1, 256, 256), device=device)
            y = init_cond
            ...
            sample, path, nfe = karras_sample(model.denoiser, model.unet,
                                                ...
                                                dc_step_kwargs={'y_meas': y_meas, 'lamda': lamda, 'sigma_meas': sigma_meas, 'step_size_scaling': step_size_scaling},
                                                clip_denoised=False
                                                )
    ...
    for i, recon in enumerate(reconstructions):
        ...
        # plt.subplot(1, 4, 2)
        # plt.imshow(fbp.cpu().squeeze(), cmap='gray')
