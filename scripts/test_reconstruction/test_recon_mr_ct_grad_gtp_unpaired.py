...
def run_experiment(
    ...
    def data_consistency(x, y_meas, diffusion, denoiser, y, sigma, sigma_next, x_T, lamda, dt, sig_t, sigma_meas, **kwargs):
        with torch.set_grad_enabled(True):
            ...
            L_each = (1 - sigma) * 1/(2*sigma_meas**2) * torch.linalg.norm(diff)**2
            L_grad = torch.autograd.grad(outputs=L_each, inputs=x)[0]

            sig_sq = 2 * lamda / (dt.abs() * torch.linalg.norm(L_grad))
    ...
    with torch.no_grad():
        for batch_idx in tqdm(range(num_batches), desc="Evaluating Batches"):
            ...
            # "Boostrapping" trick - use the first reconstruction as a new reference
    ...
    print(f"Average Initial PSNR: {avg_initial_psnr:.3f} dB")
    print(f"Average Initial SSIM: {avg_initial_ssim:.3f}")
