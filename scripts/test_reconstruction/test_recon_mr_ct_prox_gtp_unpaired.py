...
from src.models.encode_sam import get_sam_encoder, sam_encode_img, random_group_average

torch.manual_seed(123)

'''
Script for evaluating reconstruction with proximal gradient-based data consistency for the MR -> CT task.
This uses the unpaired MR encoder path together with the prox-GTP update.
'''
...
def run_experiment(
        ...
        n_cg: int = 5,
        lamda: float = 1e-3,
        num_samples: int = 1,  # Number of samples per case
    ...
    save_dir = f'{save_dir}{evaluation_set}/{task}/{n_views}_views_snr_{snr}_dB/{method}/lamda_{lamda}/n_cg_{n_cg}/'
    ...
    ckpt_path = checkpoint_file(checkpoint_root, 'mr_ct_unpaired.ckpt')

    model = CMSI.load_from_checkpoint(ckpt_path).to(device)
    model.eval()

    sam_encoder = get_sam_encoder().to(device)
    ...
    test_mr_encodings = torch.load(data_file(data_root, 'mr_ct/mr_enc', evaluation_set)).to(device)
    ...
    def A_cg(x, sigma_meas, lamda, sigma):
        return (1 - sigma) * lamda * 1 / (sigma_meas ** 2) * ct_op.A_T(ct_op.A(x)) + x

    def data_consistency(x, y_meas, diffusion, denoiser, y, sigma, sigma_next, x_T, lamda, dt, sig_t, sigma_meas, i, **kwargs):
        x_0_hat = denoiser(x, y, sigma_next.unsqueeze(0).to(device))
        A_cg_fn = lambda v: A_cg(v, sigma_meas, lamda, sigma)
        b_cg = (1 - sigma) * lamda * 1 / sigma_meas ** 2 * ct_op.A_T(y_meas) + x_0_hat
        x_dc = cg(x_0_hat, A_cg_fn, b_cg, n_iter=n_cg)
        update = x_0_hat - x_dc
        x_dc = diffusion.bridge_sample(x_dc, x_T, sigma_next.to(device))

        diff = ct_op.A(x_dc) - y_meas
        L_each = (1 - sigma) * 1 / (2 * sigma_meas ** 2) * torch.linalg.norm(diff.flatten(1), dim=1) ** 2
        L_grad = 1 / lamda * (x - x_dc)

        sig_sq = 2 * lamda / dt.abs()

        x = x - (sig_sq / 2) * dt.abs() * L_grad

        L_grad = 1 / lamda * update

        return x.detach()

    with torch.no_grad():
        for batch_idx in tqdm(range(len(test_ct_slices)), desc='Evaluating Batches'):

            mr_enc = test_mr_encodings[batch_idx].unsqueeze(0).repeat(num_samples, 1, 1, 1)
            y = random_group_average(mr_enc, n_groups=8)
            ...
            initial_reconstructions.append(sample)
            
            y = sam_encode_img(sam_encoder, sample)
            y = random_group_average(y, n_groups=8)
            init_cond = y[:, :1, :, :]
    ...
    for recon, target in zip(initial_reconstructions, test_ct_slices):
        psnr_value = compute_psnr(recon.cpu(), target.cpu().unsqueeze(0).unsqueeze(0).repeat(recon.shape[0], 1, 1, 1))
        ssim = SSIM()(recon.cpu(), target.cpu().unsqueeze(0).unsqueeze(0).repeat(recon.shape[0], 1, 1, 1))
        initial_psnr_values.append(psnr_value.item())
        initial_ssim_values.append(ssim.item())

    avg_psnr = sum(psnr_values) / len(psnr_values)
    avg_ssim = sum(ssim_values) / len(ssim_values)
    avg_initial_psnr = sum(initial_psnr_values) / len(initial_psnr_values)
    avg_initial_ssim = sum(initial_ssim_values) / len(initial_ssim_values)
    ...
    torch.save(torch.tensor(initial_psnr_values), f'{save_dir}initial_psnr_values.pt')
    torch.save(torch.tensor(avg_initial_psnr), f'{save_dir}avg_initial_psnr.pt')
    torch.save(torch.tensor(initial_ssim_values), f'{save_dir}initial_ssim_values.pt')
    torch.save(torch.tensor(avg_initial_ssim), f'{save_dir}avg_initial_ssim.pt')
    ...
    torch.save(torch.stack(initial_reconstructions).cpu(), f'{save_dir}initial_reconstructions.pt')
    ...
    with open(f'{save_dir}metrics.txt', 'w') as f:
        ...
        f.write(f'Average Initial PSNR: {avg_initial_psnr:.3f} dB\n')
        f.write(f'Average Initial SSIM: {avg_initial_ssim:.3f}\n')
