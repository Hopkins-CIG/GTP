...
class KarrasDenoiser:
    ...
    def bridge_sample_coeffs(self, t):
        ...
        # a_t * xT + b_t * x0 + std_t * noise
        return a_t, b_t, std_t
    
    def bridge_sample(self,x0, xT, t):
            dims = x0.ndim
    ...
    def training_bridge_losses(self, model, x_start, xT, y, sigmas, model_kwargs=None, noise=None, vae=None):
        ...
        model_output, denoised = self.denoise(model, x_t, y, sigmas)
    ...
    def denoise(self, model, x_t, y, sigmas):
        ...
        model_output = model(c_in * x_t, y, rescaled_t)
        denoised = c_out * model_output + c_skip * x_t
        # print('c_skip:', c_skip.mean().item(), 'c_out:', c_out.mean().item(), 'c_in:', c_in.mean().item(), 'sigma:', sigmas.mean().item(), 'rescaled_t:', rescaled_t.mean().item())
...
def karras_sample(
    ...
    device=None,
    sigma_min=0.0001,
    sigma_max=1,  # higher for highres?
    rho=7.0,
    sampler="heun",
    churn_step_ratio=0.33,
    guidance=1,
    dc_type=None, # 'x0' or 'xt'
    dc_step=None,
    dc_step_kwargs=None,
):
    ...
    dc_step_kwargs = dc_step_kwargs or {}
    dc_step_kwargs.update({"diffusion": diffusion})

    sample_fn = {
        "heun": partial(sample_heun, beta_d=diffusion.beta_d, beta_min=diffusion.beta_min,
                        dc_type=dc_type, dc_step=dc_step, dc_step_kwargs=dc_step_kwargs),
    }[sampler]
    ...
    def denoiser(x_t, y, sigma, x_T=None):
        _, denoised = diffusion.denoise(model, x_t, y, sigma)
    ...
    x_0, path, nfe = results

    if clip_denoised:
        x_0 = x_0.clamp(-1, 1)
        path = [x.clamp(-1, 1) for x in path]
...
def sample_heun(
    ...
    for j, i in enumerate(indices):
        ...
        if churn_step_ratio > 0:
            ...
            denoised = denoiser(x, y, sigmas[i] * s_in, x_T)

            if dc_type == 'x0':
                dc_step_kwargs = dc_step_kwargs or {}
                dc_step_kwargs.update({"x": x, "i": i, "sigma": sigmas[i], "denoised": denoised, "x_T": x_T, "y": y, "denoiser": denoiser,
                                        "dt": dt, "sigma_hat": sigma_hat, "guidance": guidance, "sigma_next": sigmas[i + 1]})
                denoised = dc_step(**dc_step_kwargs)
        ...
        # heun step
        denoised = denoiser(x, y, sigma_hat * s_in, x_T)
        if dc_type == 'x0':
            dc_step_kwargs = dc_step_kwargs or {}
            dc_step_kwargs.update({"x": x, "i": i, "sigma": sigmas[i], "denoised": denoised, "x_T": x_T, "y": y, "denoiser": denoiser,
                                    "dt": dt, "sigma_hat": sigma_hat, "guidance": guidance, "sigma_next": sigmas[i + 1]})
        ...
        if sigmas[i + 1] == 0: 
            ...
            # Heun's method
            x_2 = x + d * dt
            denoised_2 = denoiser(x_2, y, sigmas[i + 1] * s_in, x_T)
            if dc_type == 'x0':
                dc_step_kwargs = dc_step_kwargs or {}
                dc_step_kwargs.update({"x": x, "i": i, "sigma": sigmas[i], "denoised": denoised, "x_T": x_T, "y": y, "denoiser": denoiser,
                                        "dt": dt, "d": d, "sigma_hat": sigma_hat, "guidance": guidance, "sigma_next": sigmas[i + 1]})
                denoised = dc_step(**dc_step_kwargs)
            ...
            # noise = th.zeros_like(x) if 'flow' in pred_mode or pred_mode == 'uncond' else th.randn_like(x)
        ...
        if dc_type == 'xt':
            dc_step_kwargs = dc_step_kwargs or {}
            dc_step_kwargs.update({"x": x, "i": i, "sigma": sigmas[i], "denoised": denoised, "x_T": x_T, "y": y, "denoiser": denoiser,
                                        "dt": dt.abs(), "d": d, "sigma_hat": sigma_hat, "guidance": guidance, "sigma_next": sigmas[i + 1],
                                        "sig_t": gt2.sqrt()})
            x = dc_step(**dc_step_kwargs)
